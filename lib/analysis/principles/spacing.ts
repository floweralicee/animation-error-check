...
export function analyzeSpacing(ctx: PrincipleContext): PrincipleAnalysis {
  ...
  for (const zone of ctx.zoneProfiles) {
    ...
    const result = analyzeSpacingForFrames(zone.perFrame, fps, zone.zone, zone.displayName);
...
function analyzeSpacingForFrames(
  ...
  for (const seg of segments) {
    ...
    // Constant velocity
    if (cv < 0.15 && slice.length >= 5) {
      issues.push({
        severity: cv < 0.08 ? 'high' : 'medium',
        ...
        description: `[${zoneName}] Frames ${slice[0].frame}–${slice[slice.length - 1].frame}: constant velocity (CV: ${cv.toFixed(3)}). The ${zoneName.toLowerCase()} moves at the same speed throughout — feels mechanical.`,
        ...
        measured_data: { zone, coefficient_of_variation: Math.round(cv * 1000) / 1000, segment_length: slice.length },
        confidence: 0.78,
        body_zone: zone,
    ...
    if (firstAvg > avg * 0.8 && slice.length >= 6) {
      issues.push({
        severity: 'medium',
        frame_start: slice[0].frame,
    ...
    if (lastAvg > avg * 0.8 && slice.length >= 6) {
